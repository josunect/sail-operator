...
func TestReconcileDashboardsMissingCRDs(t *testing.T) {
	ctx := context.Background()
	cl := fake.NewClientBuilder().WithScheme(scheme.Scheme).Build()
	fsys := os.DirFS(path.Join(project.RootDir, "resources", "perses"))

	result, err := ReconcileDashboards(ctx, cl, fsys, "monitoring")
	if err != nil {
		t.Fatalf("ReconcileDashboards() error = %v", err)
	}
	if result.CRDsAvailable {
...
func TestReconcileDashboardsDedupAcrossNamespaces(t *testing.T) {
	ctx := context.Background()
	namespace := "monitoring"
	cl := newPersesTestClient(t, testPersesDashboardCRD())
	fsys := os.DirFS(path.Join(project.RootDir, "resources", "perses"))
	...
	if len(list.Items) != len(ProductDashboards) {
		t.Fatalf("expected %d dashboards after dedup reconcile, got %d", len(ProductDashboards), len(list.Items))
	}
}
