...
func ReconcileDashboards(
	...
	available, err := DashboardCRDAvailable(ctx, cl)
	if err != nil {
		return result, err
	}
...
// DashboardsExist reports whether all product dashboards exist in the target namespace.
func DashboardsExist(ctx context.Context, cl client.Client, namespace string) (bool, error) {
	for _, def := range ProductDashboards {
		existing := &unstructured.Unstructured{}
		existing.SetGroupVersionKind(DashboardGVK)
		if err := cl.Get(ctx, client.ObjectKey{Namespace: namespace, Name: def.Name}, existing); err != nil {
