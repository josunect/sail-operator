...
func main() {
	...
	err = metricsintegration.NewReconciler(reconcilerCfg, mgr.GetClient(), mgr.GetScheme()).
		SetupWithManager(mgr)
	if err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "MetricsIntegration")
		os.Exit(1)
	}
